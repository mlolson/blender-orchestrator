"""Import helpers for loading blender_addon handler modules without Blender."""
...
class _Vector(list):
    ...
    def __sub__(self, other): return _Vector([a - b for a, b in zip(self, other)])
    def __add__(self, other): return _Vector([a + b for a, b in zip(self, other)])
    def __mul__(self, scalar): return _Vector([v * scalar for v in self])
    def __rmul__(self, scalar): return self.__mul__(scalar)
    def __truediv__(self, scalar): return _Vector([v / scalar for v in self])
    def __matmul__(self, other): return other
    def __neg__(self): return _Vector([-v for v in self])
    @property
    ...
    def copy(self): return _Vector(list(self))
...
def load_handler_module(filename: str):
    """Import a single handler .py file from blender_addon/handlers/."""
    base = os.path.join(os.path.dirname(__file__), "..", "blender_addon", "handlers")
    path = os.path.join(base, filename)
    mod_name = f"blender_addon.handlers.{filename.replace('.py', '')}"
